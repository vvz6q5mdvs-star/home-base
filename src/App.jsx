...
{ id:“r2”, name:“Oats Overnight”, tags:[“breakfast”,“high protein”,“quick”], ingredients:“½ cup oats, 1 scoop whey, ½ cup milk, chia seeds, banana”, method:“Mix all ingredients. Refrigerate overnight.”, macros:{ cal:420, protein:32, carbs:48, fat:9 }, kidsFriendly:false },
{ id:“r3”, name:“Paneer Bhurji”, tags:[“breakfast”,“high protein”,“quick”], ingredients:“200g paneer, 1 onion, 1 tomato, green chilli, spices”, method:“Crumble paneer. Sauté onion, tomato, chilli. Add paneer and spices.”, macros:{ cal:380, protein:24, carbs:12, fat:22 }, kidsFriendly:true },
{ id:“r4”, name:“Vegetable Khichdi”, tags:[“lunch”,“dinner”,“kids”,“comfort”], ingredients:“½ cup rice, ¼ cup moong dal, mixed veggies, ghee, cumin, turmeric”, method:“Pressure cook rice and dal together. Add veggies and temper with ghee.”, macros:{ cal:290, protein:12, carbs:50, fat:6 }, kidsFriendly:true },
];
...
return (
<button onClick={listening ? stop : start} style={{ …S.voiceBtn, …(listening ? S.voiceBtnActive : {}), …style }} title=“Voice input”>
{listening ? “🔴” : “🎤”}
</button>
);
}
...
<VoiceBtn onResult={onChange} style={{ position:“absolute”, right:4, top:“50%”, transform:“translateY(-50%)”, width:28, height:28, fontSize:14 }} />
</div>
);
}
...
<button style={S.freeTextBtn} onClick={() => onSelect(null)}>✏️ Type freely instead</button>
</div>
</div>
);
}
...
const slots = who === “family” ? FAMILY_SLOTS : ADHYA_SLOTS;
...
if (recipe?.macros) { hasAny=true; totals.cal+=recipe.macros.cal; totals.protein+=recipe.macros.protein; totals.carbs+=recipe.macros.carbs; totals.fat+=recipe.macros.fat; }
});
...
```
  {who === "adhya" && (
    <div style={S.infoBox}>🏫 Weekday breakfast & lunch happen at daycare</div>
  )}
      ...
      <div key={day} style={S.dayCard}>
        <button style={S.dayHeader} onClick={() => setOpenDay(isOpen ? null : day)}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            {macros && <span style={S.macroSummary}>{macros.cal} kcal · {macros.protein}g protein</span>}
          ...
          <div style={S.slotsContainer}>
                ...
                <div key={slot} style={S.slotRow}>
                  ...
                  {dc ? (
                    ...
                    <div style={{ flex:1, display:"flex", gap:6, alignItems:"center" }}>
                      <TextInputWithVoice value={editVal} onChange={setEditVal} placeholder="What's cooking?" style={{ fontSize:13 }} />
                      <button style={S.saveBtn} onClick={() => { setCell(day,slot,editVal); setEditCell(null); }}>✓</button>
                    </div>
                  ) : (
                    <div style={S.cellRow}>
                      ...
                      <button style={S.recipePickBtn} onClick={() => setPicker({ day, slot })}>📖</button>
                    </div>
                  )}
                </div>
              );
            ...
            {macros && (
              <div style={S.macroBar}>
                <span style={S.macroItem}>🔥 {macros.cal} cal</span>
                <span style={S.macroItem}>💪 {macros.protein}g</span>
                <span style={S.macroItem}>🌾 {macros.carbs}g</span>
                <span style={S.macroItem}>🫙 {macros.fat}g</span>
              </div>
            )}
          </div>
        )}
...
const macros = (form.macros.cal || form.macros.protein) ? {
cal: +form.macros.cal||0, protein: +form.macros.protein||0,
carbs: +form.macros.carbs||0, fat: +form.macros.fat||0
} : null;
...
const deleteRecipe = (id) => setRecipes(r => r.filter(x => x.id !== id));
...
```
  {showForm && (
    <div style={S.formCard}>
      <div style={S.formRow}>
        <TextInputWithVoice value={form.name} onChange={v => setForm(f=>({...f,name:v}))} placeholder="Recipe name" />
      </div>
      <div style={S.formRow}>
        <TextInputWithVoice value={form.tags} onChange={v => setForm(f=>({...f,tags:v}))} placeholder="Tags: breakfast, high protein, south indian…" />
      </div>
      <div style={S.formRow}>
        <TextInputWithVoice value={form.ingredients} onChange={v => setForm(f=>({...f,ingredients:v}))} placeholder="Ingredients…" multiline />
      </div>
      <div style={S.formRow}>
        <TextInputWithVoice value={form.method} onChange={v => setForm(f=>({...f,method:v}))} placeholder="Method / steps…" multiline />
      </div>
      ...
      <label style={S.checkLabel}>
        ...
        <span style={{ marginLeft:8 }}>Kid-friendly (shows in Adhya's planner)</span>
      </label>
  ...
  {filtered.map(r => (
    <div key={r.id} style={S.recipeCard}>
      <div style={S.recipeCardHeader} onClick={() => setExpanded(expanded===r.id ? null : r.id)}>
        <div>
          <div style={S.recipeCardName}>{r.name} {r.kidsFriendly && "👧"}</div>
          <div style={S.recipePickTags}>{r.tags.map(t => <span key={t} style={S.miniTag}>{t}</span>)}</div>
        </div>
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          {r.macros && <span style={S.macroChip}>{r.macros.cal} kcal</span>}
          <span style={S.chevron}>{expanded===r.id?"▲":"▼"}</span>
        </div>
      </div>
      {expanded === r.id && (
        <div style={S.recipeBody}>
          <p style={S.recipeSection}><strong>Ingredients</strong></p>
          <p style={S.recipeText}>{r.ingredients}</p>
          <p style={S.recipeSection}><strong>Method</strong></p>
          <p style={S.recipeText}>{r.method}</p>
          {r.macros && (
            <div style={S.macroBar}>
              <span style={S.macroItem}>🔥 {r.macros.cal}</span>
              <span style={S.macroItem}>💪 {r.macros.protein}g</span>
              <span style={S.macroItem}>🌾 {r.macros.carbs}g</span>
              <span style={S.macroItem}>🫙 {r.macros.fat}g</span>
            </div>
          )}
          <button style={S.deleteBtn} onClick={() => deleteRecipe(r.id)}>🗑 Delete Recipe</button>
        </div>
      )}
    </div>
  ))}
...
```
  {lowStock.length > 0 && (
    <div style={S.alertBox}>
      ⚠️ Running low: {lowStock.map(x => x.name).join(", ")}
    </div>
  )}
  ...
  {showAdd && (
    <div style={S.formCard}>
      <div style={{ display:"flex", gap:8, marginBottom:10, flexWrap:"wrap", alignItems:"center" }}>
        <input value={form.emoji} onChange={e => setForm(f=>({...f,emoji:e.target.value}))} style={{ ...S.input, width:50, textAlign:"center" }} placeholder="🥦" />
        ...
        <div style={S.pantryGrid}>
          {items.map(item => (
            <div key={item.id} style={{ ...S.pantryCard, ...(item.qty===0?S.pantryEmpty:{}) }}>
              <button style={S.pantryDelete} onClick={() => remove(item.id)}>✕</button>
              ...
              {item.qty===1 && item.qty>0 && <span style={S.lowBadge}>Low</span>}
            </div>
          ))}
        </div>
      </div>
      ...
      <div key={d} style={{ marginBottom:20 }}>
        ...
        {dayR.map(r => (
          <div key={r.id} style={{ ...S.reminderRow, opacity:r.done?0.5:1 }}>
            <button onClick={() => toggle(r.id)} style={S.checkBtn}>{r.done?"✅":"⬜"}</button>
            <div style={{ flex:1 }}>
            ...
            <button onClick={() => del(r.id)} style={S.iconBtn}>✕</button>
          </div>
        ))}
      </div>
    );
...
{ key:“reminders”, label:“Remind”, icon:“🔔” },
];
...
recipePickBtn: { background:”#1e2535”, border:“none”, borderRadius:6, padding:“6px 8px”, cursor:“pointer”, fontSize:16 },
...
checkBtn: { background:“none”, border:“none”, cursor:“pointer”, fontSize:20, padding:0 },
...
voiceBtn: { background:”#1e2535”, border:“1px solid #2a3548”, borderRadius:6, cursor:“pointer”, fontSize:16, display:“flex”, alignItems:“center”, justifyContent:“center”, flexShrink:0 },
